/**
 * Two day graphics with something to look at, and the arrival every graphic was
 * missing.
 *
 * `arc` and `rings` were the same idea drawn twice — a line with dots on it,
 * and some concentric circles. Neither had a shape of its own, neither did
 * anything as the day filled beyond getting slightly brighter, and there was no
 * moment at the end: 99% and 100% looked the same. A day you finish should not
 * look like a day you nearly finished.
 *
 * What replaces them:
 *
 * - **`constellation`** — a star per habit, on a golden-angle spiral so no two
 *   crowd. Closing a habit lights its star, and a line is drawn to the last one
 *   lit, so the figure literally assembles itself over the course of the day.
 *   Behind it, a star field whose density is the weeks you have already kept.
 *   Complete the day and the line closes back to the first star: a finished
 *   figure, not a brighter one.
 *
 * - **`ascent`** — a ridge, a trail up it, and a marker at how far along you
 *   are. Each scheduled thing is a waypoint you pass. The sky moves with the
 *   clock rather than with the score, so the two readings stay separate: how
 *   much of the day is gone, and how much of it you have used.
 *
 * The maths is here rather than in either renderer because two hand-tuned
 * copies drift, and it is all **filter-free on purpose** — react-native-svg's
 * filter support varies by version and a missing one renders a black rectangle
 * instead of degrading. Every glow below is built from gradients and layered
 * strokes, so the phone draws the same picture as the desktop.
 */

import { GROWTH_SIZE } from "./growth.js";
