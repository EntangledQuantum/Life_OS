...
/**
 * The two original meters: a plant that grows, a sphere that fills.
 *
 * Both encode one number — today against target — where `bloom` encodes three,
 * and that was the argument for deleting them. It was a fair criticism of them
 * as the *only* option and a bad reason to remove something people liked
 * looking at, so they are choices again rather than defaults.
 *
 * Each draws the 100% state as a ghost layer *behind* the live one, so the gap
 * between where you are and a full day is visible without reading a number.
 *
 * Reached through `DayGraphic`, which is the single entry point every surface
 * uses; nothing renders this directly.
 */
